...
class EmbedARbookingrtController extends Controller
{
...
//Get Parameter pembanding
    ...
    $report_name = DB::table('embed_report')->where('report_nm','ARbookingrt')->first();
...
if ($area_id != '[]')
{
$value = DB::connection('oracle')->table('APPL.RPT_AREA_1_V')->select('AREA_ID')->where('APPL.RPT_AREA_1_V.PERSON_EMPL_ID', [Auth::user()->npk])->distinct()->pluck('area_id');
$embed_url = DB::table('embed_report')->where('report_nm','ARbookingrt')->where('report_role','A')->first();
return view('EmbedFilter1',compact('value' , 'token', 'embed_url'));
}
elseif ( $sub_area_id != '[]')
{
$value = DB::connection('oracle')->table('APPL.RPT_SUB_AREA_1_V')->select('AREA_ID')->where('APPL.RPT_SUB_AREA_1_V.PERSON_EMPL_ID', [Auth::user()->npk])->distinct()->pluck('area_id');
$embed_url = DB::table('embed_report')->where('report_nm','ARbookingrt')->where('report_role','A')->first();
return view('EmbedFilter1',compact('value' , 'token', 'embed_url'));
}
elseif ( $outlet_id == '[{"person_outlet_id":"0001"}]' )
{
$embed_url = DB::table('embed_report')->where('report_nm','ARbookingrt')->where('report_role','N')->first();
...
$embed_url = DB::table('embed_report')->where('report_nm','ARbookingrt')->where('report_role','C')->first();
return view('EmbedFilter1',compact('value' , 'token', 'embed_url'));
}
  }//
}

...
class EmbedARexcludeController extends Controller
{
...
//Get Parameter pembanding
    ...
    $report_name = DB::table('embed_report')->where('report_nm','ARexclude')->first();
...
if ($area_id != '[]')
{
$value = DB::connection('oracle')->table('APPL.RPT_AREA_1_V')->select('AREA_NAME')->where('APPL.RPT_AREA_1_V.PERSON_EMPL_ID', [Auth::user()->npk])->distinct()->pluck('area_name');
$embed_url = DB::table('embed_report')->where('report_nm','ARexclude')->where('report_role','A')->first();
return view('EmbedFilter1',compact('value' , 'token', 'embed_url'));
}
elseif ( $sub_area_id != '[]')
{
$value = DB::connection('oracle')->table('APPL.RPT_SUB_AREA_1_V')->select('SUB_AREA_NAME')->where('APPL.RPT_SUB_AREA_1_V.PERSON_EMPL_ID', [Auth::user()->npk])->distinct()->pluck('sub_area_name');
$embed_url = DB::table('embed_report')->where('report_nm','ARexclude')->where('report_role','A')->first();
return view('EmbedFilter1',compact('value' , 'token', 'embed_url'));
}
elseif ( $outlet_id == '[{"person_outlet_id":"0001"}]' )
{
$embed_url = DB::table('embed_report')->where('report_nm','ARexclude')->where('report_role','N')->first();
return view('Embed',compact('token', 'embed_url'));
}
else
{
$value = DB::connection('oracle')->table('appl.RPT_ROLE_COY_V')->select('COYOUTLET_NICK_NAME')->where('appl.RPT_ROLE_COY_V.PERSON_EMPL_ID', [Auth::user()->npk])->pluck('coyoutlet_nick_name');
$embed_url = DB::table('embed_report')->where('report_nm','ARexclude')->where('report_role','C')->first();
return view('EmbedFilter1',compact('value' , 'token', 'embed_url'));
}
  }//
}

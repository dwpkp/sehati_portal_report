...
class EmbedCollectionTrendController extends Controller
{
...
//Get Parameter pembanding
    ...
    $report_name = DB::table('embed_report')->where('report_nm','CollectionTrend')->first();
...
$value2 = DB::connection('oracle')->table('APPL.RPT_AREA_V')->select('coyoutlet_nick_name')->where('APPL.RPT_AREA_V.PERSON_EMPL_ID', [Auth::user()->npk])->distinct()->pluck('coyoutlet_nick_name');
$embed_url = DB::table('embed_report')->where('report_nm','CollectionTrend')->where('report_role','A')->first();
...
{
$embed_url = DB::table('embed_report')->where('report_nm','CollectionTrend')->where('report_role','N')->first();
...
$embed_url = DB::table('embed_report')->where('report_nm','CollectionTrend')->where('report_role','C')->first();
return view('EmbedFilter1',compact('value' , 'token', 'embed_url'));
}
    //return view('Embed',compact('value' ,'value2', 'token', 'embed_url'));

...
class EmbedARPerformanceController extends Controller
{
...
//Get Parameter pembanding
    ...
    $report_name = DB::table('embed_report')->where('report_nm','ARPerformance')->first();
...
$embed_url = DB::table('embed_report')->where('report_nm','ARPerformance')->where('report_role','A')->first();
...
{
$embed_url = DB::table('embed_report')->where('report_nm','ARPerformance')->where('report_role','N')->first();
...
$embed_url = DB::table('embed_report')->where('report_nm','ARPerformance')->where('report_role','C')->first();
return view('EmbedFilter1',compact('value' , 'token', 'embed_url'));
}
  }
}

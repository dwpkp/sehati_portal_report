...
class EmbedRppriclosingController extends Controller
{
...
//Get Parameter pembanding
    ...
    $report_name = DB::table('embed_report')->where('report_nm','Rppriclosing')->first();
...
$embed_url = DB::table('embed_report')->where('report_nm','Rppriclosing')->where('report_role','A')->first();
...
$value2 = DB::connection('oracle')->table('appl.RPT_SUB_AREA_1_V')->where('appl.RPT_SUB_AREA_1_V.PERSON_EMPL_ID',[Auth::user()->npk])->get()->pluck('coyoutlet_nick_name');
$embed_url = DB::table('embed_report')->where('report_nm','Rppriclosing')->where('report_role','A')->first();
return view('EmbedFilter2',compact('token', 'embed_url','value','value2'));
}
elseif ( $outlet_id == '[{"person_outlet_id":"0001"}]' )
{
$embed_url = DB::table('embed_report')->where('report_nm','Rppriclosing')->where('report_role','N')->first();
...
//  $value2 = DB::connection('oracle')->table('APPL.RPT_ROLE_V')->select('AREA_NAME')->where('APPL.RPT_ROLE_V.PERSON_EMPL_ID', [Auth::user()->npk])->distinct()->pluck('area_name');
  $embed_url = DB::table('embed_report')->where('report_nm','Rppriclosing')->where('report_role','C')->first();

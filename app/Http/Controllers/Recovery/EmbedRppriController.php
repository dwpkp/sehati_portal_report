...
class EmbedRppriController extends Controller
{
...
//Get Parameter pembanding
    ...
    $report_name = DB::table('embed_report')->where('report_nm','Rppri')->first();
...
$embed_url = DB::table('embed_report')->where('report_nm','Rppri')->where('report_role','A')->first();
...
{
$embed_url = DB::table('embed_report')->where('report_nm','Rppri')->where('report_role','N')->first();
...
//  $value2 = DB::connection('oracle')->table('APPL.RPT_ROLE_V')->select('AREA_NAME')->where('APPL.RPT_ROLE_V.PERSON_EMPL_ID', [Auth::user()->npk])->distinct()->pluck('area_name');
  $embed_url = DB::table('embed_report')->where('report_nm','Rppri')->where('report_role','C')->first();

...
class EmbedbqController extends Controller
{
...
//Get Parameter pembanding
    ...
    $report_name = DB::table('embed_report')->where('report_nm','bq')->first();
...
$embed_url = DB::table('embed_report')->where('report_nm','bq')->where('report_role','A')->first();
...
{
$embed_url = DB::table('embed_report')->where('report_nm','bq')->where('report_role','N')->first();
...
$embed_url = DB::table('embed_report')->where('report_nm','bq')->where('report_role','C')->first();
return view('EmbedFilter1',compact('value' , 'token', 'embed_url'));
}
  }
}

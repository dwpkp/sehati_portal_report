...
class EmbedkkhController extends Controller
{
...
//Get Parameter pembanding
    ...
    $report_name = DB::table('embed_report')->where('report_nm','kkh')->first();
...
$value2=$value;
$value3=$value;
$embed_url = DB::table('embed_report')->where('report_nm','kkh')->where('report_role','S')->first();
return view('EmbedFilter3',compact('value','value2','value3' , 'token', 'embed_url'));
}
elseif ( $sub_area_id != '[]')
{
...
$value2=$value;
$value3=$value;
$embed_url = DB::table('embed_report')->where('report_nm','kkh')->where('report_role','A')->first();
return view('EmbedFilter3',compact('value','value2','value3' , 'token', 'embed_url'));
}
elseif ( $outlet_id == '[{"person_outlet_id":"0001"}]' )
{
$embed_url = DB::table('embed_report')->where('report_nm','kkh')->where('report_role','N')->first();
...
$embed_url = DB::table('embed_report')->where('report_nm','kkh')->where('report_role','C')->first();

...
class EmbedprofilebookingController extends Controller
{
  public function index()
  {
    ...
    $report_name = DB::table('embed_report')->where('report_nm','profilebooking')->first();
...
//$value2 = DB::connection('oracle')->table('APPL.RPT_AREA_1_V')->select('AREA_NAME')->where('APPL.RPT_AREA_1_V.PERSON_EMPL_ID', [Auth::user()->npk])->distinct()->pluck('area_name');
$embed_url = DB::table('embed_report')->where('report_nm','profilebooking')->where('report_role','A')->first();
...
//$value2 = DB::connection('oracle')->table('APPL.RPT_SUB_AREA_1_V')->select('SUB_AREA_NAME')->where('APPL.RPT_SUB_AREA_1_V.PERSON_EMPL_ID', [Auth::user()->npk])->distinct()->pluck('sub_area_name');
$embed_url = DB::table('embed_report')->where('report_nm','profilebooking')->where('report_role','S')->first();
return view('EmbedFilter1',compact('value' , 'token', 'embed_url'));
}
elseif ( $outlet_id == '[{"person_outlet_id":"0001"}]' )
{
$embed_url = DB::table('embed_report')->where('report_nm','profilebooking')->where('report_role','N')->first();
...
$embed_url = DB::table('embed_report')->where('report_nm','profilebooking')->where('report_role','C')->first();
return view('EmbedFilter1',compact('value'  , 'token', 'embed_url'));
}
  }
}
